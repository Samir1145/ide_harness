...
import { BrowserWindow, app, nativeImage } from '@theia/core/electron-shared/electron';
...
export class IconContribution implements ElectronMainApplicationContribution {
    ...
    onStart(application: ElectronMainApplication): void {
        ...
        if (os.platform() === 'darwin') {
            // Set the macOS Dock icon programmatically (required in development mode
            // where electron-builder packaging is not used, so .icns is not applied automatically).
            const icon = nativeImage.createFromPath(iconPath);
            if (!icon.isEmpty()) {
                app.dock?.setIcon(icon);
            }
            // Also set the icon on any existing browser windows (e.g. splash screen)
            for (const window of BrowserWindow.getAllWindows()) {
                window.setIcon(icon);
            }
        }
