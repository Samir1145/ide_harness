...
if [ -f "$ROOT_DIR/launchers/stop.sh" ]; then
    bash "$ROOT_DIR/launchers/stop.sh" >/dev/null 2>&1 || true
fi

# Trap signals to clean up on exit
trap 'bash "$ROOT_DIR/launchers/stop.sh" >/dev/null 2>&1 || true; exit' INT TERM HUP EXIT
