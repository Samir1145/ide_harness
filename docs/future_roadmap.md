...
* **Roadmap Enhancements Absorbed:**
  ...
  - **Manual 3-Step Ingestion & Event Broker:** Decoupled ingestion into three manual, user-controlled triggers (Convert to Markdown -> Generate Search Vectors -> Run AI Enrichment) to prevent event loop blockages, SQLite database locks, and memory/OOM crashes on large files.
