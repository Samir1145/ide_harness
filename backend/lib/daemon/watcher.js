...
async function indexVectorsToSqlite(caseDir, result, profile) {
    try {
        ...
        const { getEmbedding, detectDocumentVectorType } = require('../core/llm-client');
        const vectorType = detectDocumentVectorType(relative);
        ...
        for (const item of chunksToEmbed) {
            ...
            if (vec && vec.length > 0) {
                ...
                const res = insertVector.run(relative, item.sectionTitle, item.pageNum, item.chunkIndex, item.content, buffer, vectorType);
