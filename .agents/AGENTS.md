...
## 2.1 The Sovereign Legal Assembly Line (Big Picture Architecture)
Hayagriva operates fundamentally as an **in-chamber legal document factory** rather than a generic conversational bot. 

### Data Flow & Roles:
1. **Raw Case Perimeter (Inputs):** The Insolvency Professional drops case-specific files (NCLT Admission Order, Form 1 loan sanctions, creditor claim forms, bank statements, resolution plans) into the case workspace.
2. **Contextual Engine:** Local specialized coworkers extract variables into `case_kv_dictionary.json`, index vectors, and match against the chamber's 220+ practice templates (`/Desktop/HAYAGRIVA/formats` + `carkgupta/` compendium).
3. **Four Functional Agent Archetypes:**
   - **Type A (Case-Context Builders):** Construct in-chamber administrative & statutory documents using case facts + templates (e.g., Form A Public Announcement, 1st CoC Meeting Notice & Agenda, IRP Fee & IRPC Budget Proposal under Reg 34B). 100% local, air-gapped, zero-cost.
   - **Type B (Report Synthesizers / LexAI Consumers):** Ingest external forensic dossiers dispatched by the global LexAI Desk (e.g., multi-registry director conflict checks, MCA-21 charge tracking, Section 65 collusive petition analysis) and weave them into Court-ready affidavits, Section 29A Eligibility Dossiers, and avoidance inquest applications (§§ 43, 45, 50, 66).
   - **Type C (Statutory Compliance & Legal Auditors):** Audit incoming documents (draft resolution plans, EoIs, creditor voting records) against encrypted legal databases (`vaults` / bare IBC Sections & IBBI Regulations), generating compliance audit notes (e.g. Section 30(2) checklist, Form H certification).
   - **Type D (Claim Verification & Form Fillers):** Ingest raw claims (Forms B, C, CA, D) and bank ledgers to compute admitted vs disallowed amounts and produce formal verification memos (`C2`), creditor lists (`C3`), and condonation notes (`C4`).
4. **Structured Outputs:** Generated files are saved to `drafts/`, tracked with version hashes, and immediately opened in Monaco for review, editing, and Supreme Court / DOCX export.

---

## 3. Future Roadmap Specifications
Refer to the following plans saved in the workspace:
* **Plan 1 (Asynchronous Ingestion Queue):** Enqueues ingestion jobs in a task queue to prevent database write lock collisions during concurrent document parsing. (Roadmap #30) [100% Implemented]
* **Plan 2 (Rename Sync & Hash Tracking):** Debounces `unlink` for 200ms and checks newly added file hashes to auto-rename directories instead of re-converting from scratch. (Roadmap #4) [100% Implemented]
* **Plan 3 (Clickable Citation Preview):** Maps raw LLM references (e.g. `[source:id]`) to interactive numbered links that trigger popup drawers displaying preview segments. (Roadmap #29) [100% Implemented]
* **Plan 4 (Active-Context Control Matrix):** Provides a UI checklist mapping documents to active selection states (Exclude/Insights/Full) with dynamic token count calculation. (Roadmap #28) [100% Implemented]
...
* **Plan 7 (Dual-Vector Search Loop):** Enables hybrid cases containing both legal files and financial sheets to run parallel retrievals against the correct model's vector subsets. (Roadmap #31) [100% Implemented]
* **Plan 8 (Enterprise Risk-Tiered Tool Classification):** Formalizes tool classification into `READ`, `WRITE_LOCAL`, `EXEC`, and `EXTERNAL` risk tiers to enforce path validation, snapshotting, and human authorization. (Roadmap #32) [100% Implemented]
* **Plan 9 (Case Action Inbox / Non-Blocking HITL):** Implements an asynchronous attention queue (`case_inbox.json` + Theia activity bar widget) for agent-suspended approvals and clarifications, replacing disruptive modal popups. (Roadmap #33) [100% Implemented]
* **Plan 10 (Mathematical Context Auto-Compaction):** Automatically compacts multi-turn conversational history to fit the strict 2,048-token local LLM budget (25% working memory + top 40 user prompts + 400-char tool truncation + bridging synthesis) while preserving immutable on-disk audit logs. (Roadmap #34) [100% Implemented]
* **Plan 11 (Zero-Idle Self-Wake State Machine):** Schedules statutory CIRP milestones and event-driven agent resumptions without continuous CPU polling, keeping background agent footprint at 0% CPU and 0 MB RAM when idle. (Roadmap #35) [100% Implemented]
* **Plan 12 (Wiki as Compounding Artifact):** Instead of discarding RAG answers after each session, file high-quality synthesized answers back into `wiki/insights/<slug>.md`. Maintain a machine-written `wiki/INDEX.md` catalog (one line per page: link + one-sentence summary) that RAG queries check first before hitting raw FTS5/vector indices. Additionally, after every document ingest, generate a brief `wiki/sources/<stem>.md` summary page via one small LLM call. Inspired by Karpathy's LLM-Wiki pattern and MindBase CLAUDE.md. (Roadmap #36) [100% Implemented]
* **Plan 13 (Case Wiki Lint Operation):** A dedicated "Audit Case Wiki" agent command that runs an LLM pass over all `wiki/*.md` files and `case_kv_dictionary.json`, flagging: (a) contradictions between `claims_registry.md` amounts and `avoidance_ledger.md` entries, (b) `case_facts.md` fields that are blank but derivable from ingested docs, (c) cross-references with no corresponding wiki page, (d) document chunks with zero retrieval count (orphan knowledge), and (e) CIRP statutory milestones lacking a corresponding ingested filing. Output as structured cards in the Case Action Inbox. Inspired by MindBase's lint operation ("the killer differentiator no other PKM tool has"). (Roadmap #37) [100% Implemented]
* **Plan 14 (Mechanical Working-State Extraction for Compaction):** Extend Plan 10 compaction with a hallucination-free working-state block: after each agent turn, maintain a lightweight `case_session.json` (not LLM-generated) tracking which documents were indexed, which KV keys were written, and which wiki pages were modified. On context compaction, inject this block verbatim alongside the LLM summary. Also adopt the `pick_boundary()` algorithm (prefer user-message turn boundaries; fall back to assistant boundaries; never cut mid-tool-result) and cap preserved user messages at 40 across repeated compactions. Inspired by OpenWorker `compaction.py`. (Roadmap #38) [100% Implemented]
* **Plan 15 (Context Overflow Marker Detection):** Add `is_context_overflow()` string-match detection to `llm-client.js` that intercepts provider-level 400 errors (e.g. `"context_length_exceeded"`, `"prompt is too long"`, `"too many tokens"`) and routes them directly into the compaction/fallback policy instead of surfacing raw errors to the user. Complements the existing pre-flight token count guard. Inspired by OpenWorker `compaction.py`. (Roadmap #39) [100% Implemented]
* **Plan 16 (Inbox Visibility Modes & Durable Resume):** Extend Plan 9 Case Action Inbox with two key patterns from OpenWorker `inbox.py`: (a) `VIS_INLINE` vs `VIS_INBOX` — attended sessions show approval prompts inline in the chat composer; unattended/background agent runs park items cross-session in the inbox widget without interruption; (b) suspension + durable resume — when a subagent (FormsAgent, DocumentAgent) needs approval, it writes an inbox item and suspends itself, and the coordinator resumes from the exact suspension point on approval. Also adopt idempotent resolution semantics keyed by `(session_id, tool_call_id)` to prevent race conditions when the user answers from multiple surfaces simultaneously. (Roadmap #40) [100% Implemented]
* **Plan 17 (Permission Hard Floors & Arg-Executor Blacklist):** Enumerate operations that are always human-gated regardless of auto-approve mode: (a) overwriting a `drafts/` document, (b) writing to `case_kv_dictionary.json` a key already marked `verified_by_user = 1`, (c) any network call outside localhost (critical before iPIE Gateway integration), (d) terminating a running LLM engine process. Additionally, implement compound-command splitting (`&&`, `||`, `;`, `|`) before any shell allowlist check, and blacklist arg-executor programs (`xargs`, `npx`, `docker`, `sudo`, `ssh`) from auto-execution since the actual executed program lives in their arguments. Inspired by OpenWorker `permissions.py`. Extends Plan 8. (Roadmap #41) [100% Implemented]
* **Plan 18 (Conversational Post-Ingest Brief & Interactive Emphasis Mode):** After a document is indexed, the DocumentAgent fires a 3–5 bullet "what I found" message in chat (key entities, document type, warning flags such as scanned sections or unresolved amounts). For critical case documents (Resolution Plans, CIRP applications), offer an "Enhanced Ingest" mode where the agent pauses and asks the user which focus area to prioritize (financial terms / Section 29A eligibility / Section 30(2) waterfall / all). User selection shapes chunk tagging and retrieval ranking. Encode `ingest_mode: "interactive" | "batch"` per case in `case_manifest.json`. Inspired by MindBase and Second-Brain-Skill gap analysis of "black-box one-shot" ingest. (Roadmap #42) [100% Implemented]
* **Plan 19 (Answer Filing & Wiki Insight Auto-Suggest):** Add a "Save to Case Wiki" button to every agent chat response that files the answer as `wiki/insights/<slug>.md` with frontmatter (`source_query`, `date`, `documents_used`, `agent`) and indexes it in FTS5. Additionally, auto-suggest filing when the answer cites ≥ 4 distinct documents and is ≥ 300 words (show a subtle chip in the chat UI, not an auto-file). Prioritize `wiki/insights/*.md` above raw document chunks in the `retrieveContexts` retrieval loop since compiled insights are higher-quality context than raw fragments. Inspired by Karpathy's "good answers can be filed back into the wiki as new pages." (Roadmap #43) [100% Implemented]
* **Plan 20 (CIRP Milestone Self-Wake via node-cron):** Implement Plan 11 using a lightweight `node-cron` job inside the existing `cli.js` background daemon (no separate process). On case open, read `timeline.md` milestones and register each statutory deadline ($T_0 → T_{330}$) as a scheduled wake event. When wall-clock crosses a deadline, emit a Theia notification and drop a structured card in the Case Action Inbox (deadline name, statutory reference, recommended action, one-click DocumentAgent trigger). Also add file-system-based event triggers: new file in `raw/` → ingest wake; save to `claims_registry.md` → CoC voting share recalculation wake. Store scheduled events in `case_manifest.json`. Inspired by OpenWorker `selfwake.py`. (Roadmap #44) [100% Implemented]
* **Plan 21 (Typed Entity Graph & Contradiction Edges):** Extend the VMS D3.js Case Graph Viewer and `case_kv_dictionary.json` with a lightweight SQLite `case_entities` table carrying typed directional edges: `related_party` (creditor → resolution applicant), `contradicts` (mismatched amounts across filings), `supersedes` (amended filing over original), `cites` (document referencing another). Surface contradiction edges in the Concepts sidebar when a creditor's entry is opened ("⚠️ CONTRADICTS: Claim ₹5.2Cr in ClaimForm vs ₹4.8Cr in CoC Minutes"). Render edge types with distinct colors in the D3 graph (red = contradicts, orange = related_party, blue = cites) to make the graph a diagnostic audit tool. Inspired by MindBase's typed knowledge graph ("beyond Karpathy"). (Roadmap #45) [100% Implemented]
* **Plan 22 (DeepSeek-Harness Modernization - 4 Modifications):** Modernized harness architecture with: (M1) Pluggable capability seams (`backend/lib/seams/`), (M2) Immutable fiduciary audit trail (`audit_trail.jsonl` with SHA-256 hash chaining & Section 65B/63 evidence certificates), (M3) Cordis-style LIFO lifecycle teardown and matter scoping (`backend/lib/core/lifecycle-manager.js`), and (M4) Declarative policy guard enforcing 5 hard floors, compound command splitting, and arg-executor blacklisting (`backend/lib/security/policy_guard.js`). [100% Implemented]
* **Plan 23 (Self-Evolving Agent Skills via WikiSkill Loop):** Closed-loop evolution compiling practitioner corrections, tool traces, and Monaco edits into permanent diagnostic pattern pages (`wiki/patterns/*.md`), proposing targeted patches to `.agents/skills/*` and prompt skeletons, and committing them only upon passing held-out regression tests. Inspired by WikiSkill (Google Research, arXiv:2608.27454). (Roadmap #46)
* **Plan 24 (SignalX & Section 29A Multi-Registry Due Diligence Inquest):** Automated 10-gate statutory eligibility audit (clauses a–j) across forensic audit profiles, MCA-21 director filings, and CIBIL/CRILC defaulter lists, piping ineligibility and connected person edges into the Plan 21 Typed Entity Graph and generating Court-ready Section 29A Certificates. Inspired by `Desktop/SIgnalX/` dossiers. (Roadmap #47)
* **Plan 25 (Multi-Bank Statement Forensic Parser & Ledger Normalizer):** Local multi-bank parser across SBI, HDFC, ICICI, Axis, and PNB statements extracting contra-sweeps, high-velocity round-tripping, and cash withdrawals during statutory look-back periods into `ledgers/bank_forensic_ledger.json` for avoidance inquest drafters (§§ 43, 45, 50, 66). Inspired by `github_codes/Financial Analysis/`. (Roadmap #48)
* **Plan 26 (Commercial Courts & Interlocutory Relief Drafter):** Statutory skeleton generators conforming to Commercial Courts Act, 2015 and CPC: Order 38 Rule 5 (Attachment Before Judgment), Order 39 Rules 1 & 2 (Temporary Injunction), and Section 12A Pre-Institution Mediation tracking. Inspired by `Desktop/Commercial Court Rules and Forms.pdf`. (Roadmap #49)
* **Plan 27 (FrameCode Scoped Drafting Memory & Matter Session Sandboxing):** Declarative workspace and context boundary enforcement isolating LLM working memory, vector caches, and KV dictionaries per matter directory (`HAYA_MATTERS/*`), preventing cross-matter fact bleeding or hallucination. Inspired by `FrameCode-VibeWork`. (Roadmap #50)
...
* **DeepSeek Harness Capability Seams Architecture (Modification 1 - Completed)**:
  * Inspired by DeepSeek's `deepseek-ai/deepseek-harness` 3-role seam design:
    * **Service Definition**: Defines domain capability contracts without binding to execution runtime (`backend/lib/seams/compliance/compliance-service.js`, `backend/lib/seams/llm/llm-service.js`).
    * **Service Providers**: Encapsulate concrete runtimes behind standardized provider interfaces (`executeTask(task, context)`, `healthCheck()`, `getCapabilities()`):
      - `LocalSubagentProvider`: Dispatches tasks to in-chamber deterministic subagents (`@statutory_auditor`, `@document`).
      - `LexAICloudProvider`: Dispatches tasks to LexAI Cloud Desk (port 4000) with offline fallback staging and TiddlyWiki ingestion.
    ...
    * **Consumer**: Applications and HTTP routes (`backend/lib/routes.js`) talk exclusively to the Seam Service, eliminating tight coupling and conditional branches.
* **Electron Crashpad Loop & Scratch Hygiene (Incident & Prevention)**:
  * **Root Cause**: Extracting multi-thousand file repositories (e.g. 13,850 files from `deepseek-harness`) into the agent scratch directory (`.gemini/antigravity-ide/brain/.../scratch/`) overwhelms Electron's file watchers, TypeScript language server (`tsserver.js`), and Chromium's embedded `chrome_crashpad_handler`. Crashpad encounters `EXC_BREAKPOINT` / `SIGTRAP` in Rust logging, triggering macOS's `ReportCrash` daemon to continuously show crash prompt dialogs.
  * **Hygiene Rule**: Never unpack large codebases (100+ files) directly into scratch. Always compress and preserve as `.tar.gz` (`scratch/deepseek-harness.tar.gz`) and inspect archives on demand via tar/unzip pipes.
  * **Duplicate Process Guard**: Always verify only one `node cli.js --watch-all` daemon runs on port 3210. Kill orphan background watchers parented to `init (PID 1)` to prevent chokidar double-event loops and database contention.
* **Plan 22 (DeepSeek-Harness 4 Architectural Modifications)**:
  * **Modification 1: Pluggable Capability Seams** (Completed in `backend/lib/seams/`).
  * **Modification 2: Immutable Fiduciary Audit Trail (`audit_trail.jsonl`)** (Completed in `backend/lib/core/audit_trail.js`):
    - Append-only SHA-256 hash-chained ledger (`prev_hash` -> `curr_hash`) with serialized Promise write queue (`_writeQueues`) per case to eliminate race conditions.
    - Captures monotonic `seq`, ISO timestamp, actor (`@statutory_auditor`, `@document`, `HUMAN_IP`), inputs digest, generated artifact fingerprints, and legal verdicts.
    - Integrated with `LocalSubagentProvider`, `LexAICloudProvider`, `AirGappedStagedProvider`, and `InboxManager` on HITL item approvals/dismissals.
    - Exposes `GET /api/hayagriva/case/audit-trail`, `GET /api/hayagriva/case/audit-trail/verify`, and `GET /api/hayagriva/case/audit-trail/export` generating court-admissible certificates under Section 65B of Indian Evidence Act / Section 63 of BSA 2023.
  * **Modification 3: Cordis-Style Reversible Effect Lifecycles & Teardown Protocol** (Completed in `backend/lib/core/lifecycle-manager.js`):
    - Implemented `MatterScope` and `MatterLifecycleManager` inspired by Cordis disposal fibers.
    - Tracks and safely tears down all reversible effects (Chokidar watchers, timers, intervals, event listeners, child processes, open SQLite handles) in strict LIFO (reverse) order.
    - Provides clean matter switching (`switchMatter(fromCaseDir, toCaseDir)`) ensuring the previous case releases all DB write locks (`SQLITE_BUSY` prevention) and file watchers before the new case starts.
    - Exposes `GET /api/hayagriva/matter/lifecycle-status`, `POST /api/hayagriva/matter/switch`, and `POST /api/hayagriva/matter/teardown`.
    - Integrated with `backend/cli.js` process exit signal handlers (`SIGINT`, `SIGTERM`).
  * **Modification 4: Declarative Risk-Tiered Policy Guard & Hard Floors** (Completed in `backend/lib/security/policy_guard.js`):
    - Implemented `PolicyGuard` enforcing the 5 inviolable Hard Floors (Plan 17 & Plan 8):
      1. `MUTATE_VERIFIED_KEY`: Blocks overwriting any `case_kv_dictionary.json` fact where `verified_by_user = 1`.
      2. `OVERWRITE_DRAFT`: Blocks overwriting existing legal pleadings in `drafts/` without snapshotting/confirmation.
      3. `EXTERNAL_NETWORK`: Enforces air-gap boundary; blocks egress to non-localhost addresses without explicit authorization.
